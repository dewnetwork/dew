...
import { classifySearch, truncateHex } from "@/lib/format";
...
export function SearchBox({ className = "" }: { className?: string }) {
  ...
  async function goTo(raw: string) {
    setError(null);
    ...
    <div className={`w-full ${className}`}>
      ...
      {recent.length > 0 ? (
        <div className="mt-2 flex flex-wrap items-center gap-1.5" aria-label="Recent searches">
          <span className="text-[10px] tracking-wide text-muted uppercase">Recent</span>
          {recent.map((item) => (
            <button
              key={item}
              type="button"
              ...
              className="mono surface-sm max-w-[12rem] truncate border border-[var(--color-line)] px-2 py-0.5 text-[11px] text-slate transition hover:border-cyan hover:text-cyan"
              title={item}
            >
              {item.length > 18 ? truncateHex(item, 6, 4) : item}
            </button>
          ))}
        </div>
      ) : null}
